      ...
      <div className="mx-auto max-w-6xl px-6 py-20">
        ...
        <div className="mt-12 grid gap-8 sm:grid-cols-2 lg:grid-cols-4">
          {DIFERENCIALES.map((item) => (
            <Reveal key={item.title}>
              <div
                className={`rounded-2xl border-x border-b border-t-4 border-black/10 bg-background p-6 dark:border-white/10 ${item.accent}`}
              >
        ...
        <Reveal>
          <p className="mt-12 max-w-2xl text-foreground/70">
            Nuestro compromiso es ayudarte a que cada pieza comunique la
            calidad y el prestigio de tu marca.
          </p>
        </Reveal>
      </div>
    </section>
